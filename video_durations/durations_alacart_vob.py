''' Used to save VOB video durations from ALACART extracted manually.
'''
# %reset -f
import numpy as np
import datetime
...
# 'vob' contains the names of the files whose duration was found out manually
# and their respective durations (in DD:HH:MM format)
...
path = np.array(np.repeat('SONY_DVD_RECORDER_VOLUME/VIDEO_TS/', len(vob), 0),
                dtype=object)

# Changing the durations in 'vob' to timedelta format (for easier working with
# later on)
for row in range(len(vob)):
    vob[row, 1] = datetime.timedelta(hours=int(vob[row, 1][0:2]),
                                     minutes=int(vob[row, 1][3:5]),
                                     seconds=int(vob[row, 1][6:8]))
...
video = np.transpose(np.vstack([path, vob[:, 0], vob[:, 0], vob[:, 1],
                                np.full(len(vob), np.nan)]))
