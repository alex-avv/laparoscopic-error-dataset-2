...
# 'extension' will be used to retrieve the files with either of the video
# extensions given and their location within the chosen folder later on
...
def store_files_name(fname):
    ...
    if fname[-4:len(fname)] in extension:
        ...
        for fname_last_position in reversed(range(len(fname))):
...
# Scanning all of the files in the chosen SFTP folder and storing information
# about those which are videos
sftp.walktree(".", store_files_name, emptyFunction, emptyFunction,
              recurse=True)
print(file_name)
...
# 'video' will keep the Path of the file, the Original name, the Short form
# name, the Duration and the Global start time of each video
video = np.transpose(np.vstack((path, file_name, file_name,
                                np.full((2, len(file_name)), np.nan))))

# SERA'S CODE
# ===========
# with sftp.cd('/cs/research/medic/surgicalvision/srv6/Griffin_dataset/'
#              'ALACART/All Lap surgical Videos'):
...
# 'substitute' contains the characters which have to be replaced from the
# Original name of the file to get the Short form name. A single string in an
# entry of the dictionary deletes those characters later on. An array with two
# strings in an entry of the dictionary replaces the characters of the first
# with the characters of the second. For example, '009':['0',['_','.']] deletes
# zeroes and replaces underscores with full stops
substitute = {'009': ['0'], '013': ['0'], '016': ['0'], '022': ['0'],
              '026': ['0'], '033': ['0'],
              '070': ['ANDREWS_JAMES_Laparoscopic surg_20120329_'],
              '074': ['0'], '077': ['RA 077 - video ', 'RA 077 video '],
              '080': ['ch1_video_0'], '087': ['0'], '098': ['0000000'],
              '099': ['ch1_video_0'], '115': ['ch1_video_0'],
              '130': ['ch1_video_0'], '132': ['ch2_video_0', 'pip_video_0'],
              '137': ['0000000'], '144': ['0'], '146': ['PDH 146 - video '],
              '147': ['0'], '150': ['0'], '153': ['ch1_video_0'], '157': ['0'],
              '161': ['ch1_video_0'], '162': ['ch1_video_0'],
...
for row in range(len(video)):
    ...
    for m in range(len(substitute[folder_name])):
        # Removing first part of file name
        if len(substitute[folder_name][m]) == 2:
            video[row, 2] = video[row,2].replace(substitute[folder_name][m][0],
                                                 substitute[folder_name][m][1])
        else:
            video[row, 2] = video[row,2].replace(substitute[folder_name][m],'')
    for n in range(len(extension)):
...
video[0, 4] = datetime.timedelta(seconds=0)
...
os.chdir('C:/Users/aleja/OneDrive - University College London/Griffin '
         'Institute collaboration/Grifin_annotations/ALACART/Video durations')

# Saving video data as .npy file
np.save(f'{folder_name}', video)

# Loading a single .npy file (for testing)
exec("dur_" + folder_name + " = np.load(f'{folder_name}.npy', allow_pickle="
     "True)")
